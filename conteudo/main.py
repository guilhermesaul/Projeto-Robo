...
while rodando:
    ...
    if estado == "menu":
        ...
        texto_iniciar = fonte_opcao.render(
            "► INICIAR" if opcao_menu == 0 else "  INICIAR", True, cor_iniciar
        )
        texto_sair = fonte_opcao.render(
            "► SAIR" if opcao_menu == 1 else "  SAIR", True, cor_sair
        )
        ...
        info = fonte_info.render(
            "Use SETAS e ENTER para navegar", True, (150, 150, 150)
        )
        ...
        TELA.blit(
            texto_vitoria,
            (LARGURA // 2 - texto_vitoria.get_width() // 2, ALTURA // 2 - 120),
        )
        TELA.blit(
            texto_pontos,
            (LARGURA // 2 - texto_pontos.get_width() // 2, ALTURA // 2 - 40),
